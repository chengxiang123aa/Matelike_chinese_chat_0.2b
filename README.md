...
## 2.1 预训练数据集
所有数据集均来自modelscope社区的大佬**出门问问**分享的**seq-monkey 序列猴子开源数据集 1.0**(https://modelscope.cn/datasets/ddzhu123/seq-monkey)，序列猴子是出门问问提供的超大规模语言模型，序列猴子数据集是用于训练序列猴子模型的数据集合，数据集涉及领域包括：中文通用文本语料、古诗今译语料、文本生成语料，不需要清洗，非常nice的json格式文件。
